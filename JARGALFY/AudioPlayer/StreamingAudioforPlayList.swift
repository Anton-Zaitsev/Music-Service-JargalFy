//
//  StreamingAudioforPlayList.swift
//  JARGALFY
//
//  Created by Антон Зайцев on 11.12.2020.
//
...
struct StreamingAudioforPlayList: View {
    @EnvironmentObject  var DataMedia :  PlayListDataClick
    ...
    @State var redGradient : Int = Int.random(in: 0...255)
    @State var greenGradient: Int = Int.random(in: 0...255)
    @State var blueGradient: Int = Int.random(in: 0...255)
    let spacing: CGFloat = 10
    let withScreen  = UIScreen.main.bounds.width-30
    let user = Auth.auth().currentUser
    let db = Firestore.firestore()
    var body: some View {
        if self.DataMedia.DataPlayListClick.count != 0{
            VStack{
                HStack{
                    if !self.show {
                        ForEach(self.DataMedia.DataPlayListClick){ i in
                            VStack{
                                ...
                                ZStack (alignment: .bottom){
                                    ...
                                    VStack{
                                        HStack {
                                            ...
                                            Spacer()
                                            Text(i.NamePlayList).fontWeight(.bold).foregroundColor(Color(red: 179.0/255.0, green: 179.0/255.0, blue: 179.0/255.0)).font(.system(size: UIScreen.main.bounds.width/23))
                                            Spacer()
                ...
                addRandomModifiers()
                for i in DataMedia.DataPlayListClick {
                    if boolNextAuido{
